...
const AllMagazines = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const response = await fetch(`${port}/api/magazines`);
  ...
  const handleDelete = async (userId) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${port}/api/magazines/${userId}`, {
          method: 'DELETE'
        });
        if (response.ok) {
  ...
  const handleUpdate = async (updatedBlog) => {
    try {
      const response = await fetch(`${port}/api/magazines/${updatedBlog.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Error updating Magazine:', response.status);
      }
    } catch (error) {
        ...
        {filteredData.map((item) => (
          <div className="admin_articles_list m-3 box-shadow p-3" key={item.id}>
                ...
                {editingUserId === item.id ? (
                  ...
                  <>
                  <Link href={item.flipbook_link}>
                    <img src={`${port}/uploads/magazines/${item.image}`} width={60} height={60}></img>
                    </Link>
                    <Card.Title >
                     <Link href={item.flipbook_link} target='_blank'>{item.title}  </Link>
                    </Card.Title>
